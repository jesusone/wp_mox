...
class Zo_Flickr_Badges_Widget extends WP_Widget {
	...
	**/			
	function __construct() {
		...
		$widget_options = array('classname' => 'widget_flickr', 'description' => __( 'Displays a Flickr photo stream from an ID', 'mox' ) );
	...
	**/		
	function form( $instance ) {
		...
		$defaults = array(
			'title'			=> esc_attr__( 'Flickr Widget', 'mox' ),
			'type'			=> 'user',
		...
		$types = array( 
			'user'  => esc_attr__( 'user', 'mox' ),
			'group' => esc_attr__( 'group', 'mox' )
		);
		$sizes = array(
			's' => esc_attr__( 'Standard', 'mox' ),
			't' => esc_attr__( 'Thumbnail', 'mox' ),
			'm' => esc_attr__( 'Medium', 'mox' )
		);
		$displays = array( 
			'latest' => esc_attr__( 'latest', 'mox' ),
			'random' => esc_attr__( 'random', 'mox' )
		);
		...
		<div id="fbw-<?php echo esc_attr($this->id) ; ?>" class="totalControls tabbable tabs-left">
			...
			<ul class="tab-content">
				<li class="tab-pane <?php if ( $instance['tab'][0] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							...
							<span class="controlDesc"><?php _e( 'Give the widget title, or leave it empty for no title.', 'mox' ); ?></span>
							<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php _e( 'Type', 'mox' ); ?></label>
							<span class="controlDesc"><?php _e( 'The type of images from user or group.', 'mox' ); ?></span>
						...
						<li>
							...
							<span class="controlDesc"><?php printf( __( 'Put the flickr ID here, go to <a href="%s" target="_blank">Flickr NSID Lookup</a> if you don\'t know your ID. Example: 71865026@N00', 'mox' ), esc_url('http://goo.gl/PM6rZ')); ?></span>
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php _e('Number', 'creativ'); ?></label>
							<span class="controlDesc"><?php _e( 'Number of images shown from 1 to 10', 'mox' ); ?></span>
						...
						<li>
							...
							<span class="controlDesc"><?php _e( 'Get the image from recent or use random function.', 'mox' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('sizes')); ?>"><?php _e( 'Sizes', 'mox' ); ?></label>
							<span class="controlDesc"><?php _e( 'Represents the size of the image', 'mox' ); ?></span>
				...
				<li class="tab-pane <?php if ( $instance['tab'][1] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('intro_text')); ?>"><?php _e( 'Intro Text', 'mox' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text before the widget content and HTML supports.', 'mox' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('outro_text')); ?>"><?php _e( 'Outro Text', 'mox' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text after widget and HTML supports.', 'mox' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('custom')); ?>"><?php _e( 'Custom Script & Stylesheet', 'mox' ) ; ?></label>
							<span class="controlDesc"><?php _e( 'Use this box for additional widget CSS style of custom javascript. Current widget selector: ', 'mox' ); ?><?php echo '<tt>#' . $this->id . '</tt>'; ?></span>
